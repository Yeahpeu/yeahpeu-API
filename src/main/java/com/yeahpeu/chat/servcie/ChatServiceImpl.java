...
public class ChatServiceImpl implements ChatService {
    ...
    public ChatRoomDTO createRoom(CreateRoomCommand command) {
        ...
        if (command.getUserIds().size() == 1) {
            chatRoomEntity.setReservedMemberCount(2);
        } else {
            chatRoomEntity.setReservedMemberCount(99);
        }
        ...
        List<Long> userIds = new ArrayList<>(command.getUserIds());
        userIds.add(command.getCreatorId()); // creatorId 추가

        joinRoom(new JoinChatRoomCommand(newChatRoom.getId(),userIds));

        return new ChatRoomDTO(newChatRoom.getId(), newChatRoom.getTitle());
    }

    @Override
    @Transactional(readOnly = true)
    public List<ChatRoomDetailDTO> getChatRooms(Long userId){
        ...
        for (ChatRoomUserEntity chatRoomUserEntity : chatRoomUserEntities) {
            ...
            Long unseenMessageCount = chatRoomMessageCounter.getLastMessageId() - chatRoomUserEntity.getLastReadMessageId();
            chatRoomDetailDTOS.add(new ChatRoomDetailDTO(chatRoomEntity.getId(), chatRoomEntity.getTitle(),
                    unseenMessageCount,
    ...
    public void leaveRoom(LeaveChatRoomCommand command) {
        chatRoomUserRepository.deleteByUser_IdAndChatRoom_Id(
    ...
    public ChatRoomDTO joinRoom(JoinChatRoomCommand command) {
        ...
        // 1. UserEntity 검색
        System.out.println("command ids = " + command.getUserIds());
        List<UserEntity> users = userRepository.findByIdIn(command.getUserIds());
        System.out.println("users.stream().map(UserEntity::getId).toList() = " + users.stream()
